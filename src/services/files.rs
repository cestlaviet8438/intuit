...
///
/// This struct has several options for handling the most basic needs for
/// configurations and assets and is mostly a utility for retrieving hardcoded
/// paths conveniently.
#[derive(Debug, Clone)]
...
impl AppFiles {
	...
	fn get_env_var_dir(var: &str) -> crate::Result<PathBuf> {
		...
		Self::get_existing_path(path)
			.map_err(|err| err.with_note(|| format!("read from env var: {var}")))
	}
		...
		F: Fn(&ProjectDirs) -> &Path,
	{
		Ok(match (Self::get_env_var_dir(env_folder_var), &self.0) {
			(Ok(env_path), _) => (env_path, PathSource::EnvVar(env_folder_var.to_string())),
			(_, Some(project_dirs)) => (
	...
	/// Gets a path to be used for the current app session.
	#[tracing::instrument(name = "get-app-path", skip(self, get_project_dir_path))]
	...
	/// not exist.
	#[expect(unused, reason = "really thought this would be used somewhere lol")]
		...
		F: Fn(&ProjectDirs) -> &Path,
	{
		self.get_app_path(env_folder_var, get_project_dir_path, subdir)
			...
			.and_then(|(path, source)| Ok((Self::get_existing_path(path)?, source)))
	}
		...
		F: Fn(&ProjectDirs) -> &Path,
	{
		self.get_app_path(env_folder_var, get_project_dir_path, subdir)
			.wrap_err("io error while retrieving app dir")
			.and_then(|(path, source)| Ok((Self::create_dirs_if_nonexistent(path)?, source)))
	}
	...
	{
		let (path, source) =
			self.get_or_create_app_path(env_folder_var, get_project_dir_path, subdir)?;
		let path_display = path.display().to_string();
		tracing::info!(
			%source,
			path = path_display,
	...
	pub fn get_config_path(&self, subdir: Option<PathBuf>) -> crate::Result<PathBuf> {
		self.get_or_create_app_path(&CONFIG_FOLDER_ENV_VAR, |dirs| dirs.config_dir(), subdir)
			.map(|(path, _)| path)
	}

	/// [Gets or creates](Self::get_or_create_app_path) a path from the app's
	/// [data directory](ProjectDirs::data_dir).
	pub fn get_data_path(&self, subdir: Option<PathBuf>) -> crate::Result<PathBuf> {
		self.get_or_create_app_path(&DATA_FOLDER_ENV_VAR, |dirs| dirs.data_dir(), subdir)
			.map(|(path, _)| path)
	}
	...
	pub fn get_asset_path(&self, path: PathBuf) -> crate::Result<PathBuf> {
		Self::get_existing_path(self.get_data_path(Some(".assets".into()))?.join(path))
	}
	...
	pub fn log_project_dirs(&self) -> crate::Result<()> {
		...
		self.find_app_path("data", &DATA_FOLDER_ENV_VAR, |dirs| dirs.data_dir(), None)?;
		Ok(())
	}
}
